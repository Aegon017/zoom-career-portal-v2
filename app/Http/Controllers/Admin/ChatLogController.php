...
final class ChatLogController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $query->when($request->student, function ($q) use ($request): void {
            $q->whereHas('participants.user.roles', function ($q) use ($request): void {
                $q->where('users.id', $request->student)->where('roles.name', 'jobseeker');
            });
        });

        $query->when($request->employer, function ($q) use ($request): void {
            $q->whereHas('participants.user.roles', function ($q) use ($request): void {
                $q->where('users.id', $request->employer)->where('roles.name', 'employer');
            });
        });

        $query->when($request->start_date, function ($q) use ($request): void {
            $q->whereHas('messages', function ($q) use ($request): void {
                $q->whereDate('created_at', '>=', $request->start_date);
            });
        });

        $query->when($request->end_date, function ($q) use ($request): void {
            $q->whereHas('messages', function ($q) use ($request): void {
