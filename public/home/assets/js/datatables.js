		...
		/** @lends <global> */ function ($) {
				...
				if (src === "dom" || ((!src || src === "auto") && row.src === "dom")) {
					...
					if (cells) {
						for (i = 0, ien = cells.length; i < ien; i++) {
							...
							cells[i].innerHTML = _fnGetCellData(
								settings,
								rowIdx,
								i,
								"display",
			...
			function _fnScrollDraw(settings) {
				...
				if (scroll.bCollapse && scrollY !== "") {
					divBodyStyle.height =
						divBody[0].offsetHeight + header[0].offsetHeight + "px";
				}
				...
				if (scrollX === "") {
					...
					if (scrollXInner !== "") {
						...
						sanityWidth == divBody.width() &&
						divBody.height() < table.height()
					) {
				...
				if (scrollY && scroll.bCollapse) {
					...
					var iExtra =
						scrollX && tableEl.offsetWidth > divBodyEl.offsetWidth
							? barWidth
							: 0;
			...
			DataTable = function (options) {
				...
				this.each(function () {
					...
					if (rowOne.length) {
						...
						$.each(
							_fnGetRowElements(oSettings, rowOne[0]).cells,
							function (i, cell) {
			...
			_api_register(
				["cells().invalidate()", "cell().invalidate()"],
				function (src) {
					var selector = this.selector;
