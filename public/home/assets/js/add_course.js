$(document).ready(function () {
	...
	$("body").on("click", ".btn-course-update", function () {
		...
		$("#edit_sub_course_of option[value='" + parent + "']").prop(
			"selected",
			true,
		);
	});
	...
	$("body").on("click", ".btn-employer-delete", function () {
		...
		if (empDel) {
			...
			console.log(empId);
			$.post(
				APP_URL + "welcome/delete_employe",
				{
					empId: empId,
				},
				function (response) {
				...
				if (response.status == 200) {
					$.each(response.company, function (k, v) {
						content +=
							...
							v.designation +
							"</td>\n\
                            <td align='center'><a href='javascript:void(0);' data-id=" +
							v.id +
							" class='glyphicon glyphicon-pencil btn-employer-update' title='Edit'></a>&nbsp;&nbsp;\n\
                            <a href='javascript:void(0);' data-id=" +
							v.id +
							" class='glyphicon glyphicon-trash btn-employer-delete' title='Delete'></a>&nbsp;\n\
                             </td>";
