...
$(document).ready(function () {
	...
	$("#job-seeker-form").validate({
		...
		submitHandler: function (form) {
			...
			$.post(
				APP_URL + "registration/addNewJobseeker",
				{
					userName: userName,
					email: email,
					mobile: mobile,
	...
	$("#employer_form").validate({
		...
		submitHandler: function (form) {
			...
			$.post(
				APP_URL + "registration/addNewJobseekerEmployer",
				{
	...
	$("#job-seeker-profile-form").validate({
		...
		submitHandler: function (form) {
			...
			$.post(
				APP_URL + "jobseeker/jobseeker_update",
				{
					name: name,
					email: email,
					mobile: mobile,
	...
	$("#table_box").dataTable({
		...
		fnInfoCallback: function (oSettings, iStart, iEnd, iMax, iTotal, sPre) {
			$("#checkAll,.checkbox1").prop("checked", false);
		},
		dom: "Bfrtip",
		buttons: [
			{
			...
			success: function (data) {
				...
				if (data["status"] === 1000) {
					...
					$.each(data["data"], function (k, v) {
						...
						if (v["status"] == "active") {
							textt +=
								'<td><input type="checkbox" class="checkbox1" name="check[]" value="' +
								v["email"] +
								'"></td>';
						} else {
						...
						textt +=
							'<td style="display:none">' + v["years_in_curr_job"] + "</td>";
						...
						textt += "<td>" + v["resume_date"] + "</td>";
						if (v["resume_name"] == "") {
							textt +=
								'<td><a href="javascript:void(0);" class="glyphicon glyphicon-eye-open btn-document-new-modals"></a>\n\
							...
							textt +=
								...
								"resumes/" +
								v["resume_name"] +
								'" target="blank" class="glyphicon glyphicon-paperclip" title="Resume"></a></td>';
						}
						if (v["status"] == "active") {
	...
	$("body").on("change", "#to_mail", function () {
		...
		$.post(
			APP_URL + "welcome/get_employee_details",
			{
				to_mail: to_mail,
			},
			function (response) {
	...
	$(".send-detail-form").validate({
		...
		submitHandler: function (form) {
			...
			$.post(
				APP_URL + "welcome/send_mail_list_to_emp",
				{
					to_mail: to_mail,
					emp_name: emp_name,
	...
	$("body").on("click", "#send_mails_to_jobseeker", function () {
		...
		$(".checkbox1:checked").each(function () {
			var row = $(this).closest("tr");
			mail_array.push($(row).find("td:eq(4)").text());
		});
	...
	$(".send-mail_to_seeker-form").validate({
		...
		submitHandler: function (form) {
			...
			$.post(
				APP_URL + "welcome/send_mails_to_jobseeker",
				{
					subject: subject,
					message: message,
					mail_array: mail_array,
