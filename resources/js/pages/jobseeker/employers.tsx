...
const Employers = () => {
	const {
		companies: initialCompanies = { data: [], next_page_url: null },
		filters = {},
	} = usePage().props as any;
	const [companies, setCompanies] = useState(initialCompanies.data);
	const [nextPageUrl, setNextPageUrl] = useState(
		initialCompanies.next_page_url,
	);
	const [loading, setLoading] = useState(false);
	const [activeDropdown, setActiveDropdown] = useState<
		"industries" | "locations" | "sizes" | null
	>(null);
	...
	const getUniqueValues = useCallback(
		(keyPath: string): string[] => {
			...
			return Array.from(new Set(values) as Set<string>).sort((a, b) => {
				...
				if (aMatch && bMatch) {
					const aLow = Number(aMatch[1]),
						aHigh = Number(aMatch[2]);
					const bLow = Number(bMatch[1]),
						bHigh = Number(bMatch[2]);
	...
	const toggleArray = (
		field: "industries" | "locations" | "sizes",
		value: string,
	) => {
		...
		return (
			<div className="zc-accordion-item">
				...
				<div className="zc-accordion-content">
					<div
						className="zc-filter-search-dropdown position-relative"
						ref={field === activeDropdown ? dropdownRef : null}
					>
						{data[field].length > 0 && (
									...
									<div className="tag" key={item} data-value={item}>
										...
										<i
											...
											onClick={() =>
												setData(
													field,
													data[field].filter((i: string) => i !== item),
												)
											}
										/>
									</div>
						...
						<div className="search-field d-block position-relative w-100">
							<input
								type="text"
						...
						<div
							className={`filter-dropdown-wrapper${activeDropdown === field ? "active" : ""}`}
						>
											...
											<div className="zc-accordion-content">
												<div className="zc-field check-box-field">
													<input
														...
														onChange={(e) =>
															setData("followed", e.target.checked)
														}
													/>
													<label htmlFor="followed-employers">
