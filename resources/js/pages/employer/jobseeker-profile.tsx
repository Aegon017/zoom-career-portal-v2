                    ...
                    <div className="absolute -bottom-16 sm:-bottom-16 left-4 sm:left-6 flex flex-col sm:flex-row items-center sm:items-end">
                        ...
                        <div className="mt-2 sm:ml-4 sm:mb-2 text-center sm:text-left">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground">{ user.name }</h1>
                            </div>
                            ...
                            <div className="mt-1 sm:mt-2 text-xs sm:text-sm text-muted-foreground">
                                <span className="flex justify-center sm:justify-start items-center gap-1">
                                    ...
                                    Profile last updated - { user.profile?.updated_at ? format( new Date( user.profile.updated_at ), 'dd MMM yyyy' ) : 'N/A' }
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-muted rounded-lg p-3 sm:p-4">
                                    ...
                                    { resume?.resume_url && (
                                        <a href={ resume.resume_url } className="mt-2 sm:mt-0 w-full sm:w-auto">
                                            <Button variant="outline" className="w-full sm:w-auto">
                                                <FileText className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5" /> View
                                            </Button>
                                        </a>
                                    ) }
                                </div>
                                    ...
                                    user.work_experiences.map( ( workExperience, index ) => (
                                        <div key={ index }>
                                            ...
                                            <div className="text-foreground text-sm sm:text-base mt-1">
                                                { workExperience?.company_name || workExperience?.company?.name || 'Unknown company' }
                                            </div>
                                            { index < user.work_experiences.length - 1 && <Separator className="my-3 sm:my-4" /> }
                                        </div>
                                    ) )
                                ) : (
                                ...
                                { user.educations?.length > 0 ? (
                                    user.educations.map( ( education, index ) => (
                                        <div key={ index }>
                                            ...
                                            { index < user.educations.length - 1 && <Separator className="my-3 sm:my-4" /> }
                                        </div>
                                    ) )
                                ) : (
                                    <p className="text-muted-foreground text-sm">No education history</p>
                                ) }
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="space-y-4 sm:space-y-6">
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-3 sm:space-y-4">
                                ...
                                <DetailItem
                                    label="Date of Birth"
                                    value={ user.personal_detail?.date_of_birth
                                        ? format( new Date( user.personal_detail.date_of_birth ), 'dd MMM yyyy' )
                                        : 'Not specified' }
                                />
                                ...
                                <DetailItem
                                    ...
                                    value={ user.personal_detail?.marital_status || 'Not specified' }
                                />
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-3 sm:space-y-4">
                                ...
                                <ContactItem
                                    icon={ <CalendarDays className="h-4 w-4 sm:h-5 sm:w-5 text-primary" /> }
                                    label="Notice Period"
                                    ...
                                    user.user_languages!.map( ( userLanguage ) => (
                                        <div key={ userLanguage.id }>
                                            ...
                                            <div className="flex flex-wrap items-center gap-1 sm:gap-2 mt-1 text-xs sm:text-sm text-muted-foreground">
                                                <span>{ userLanguage?.proficiency || 'Not specified' }</span>

                                                { ( userLanguage.can_read || userLanguage.can_write || userLanguage.can_speak ) && (
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        { userLanguage.can_write && (
                                                            <>
                                                                { userLanguage.can_read && (
                                                                    <Separator orientation="vertical" className="h-3 bg-muted-foreground mx-1" />
                                                                ) }
                                                                <span>Write</span>
                                                            </>
                                                        ) }
                                                        ...
                                                        { userLanguage.can_speak && (
                                                            <>
                                                                { ( userLanguage.can_read || userLanguage.can_write ) && (
                                                                    <Separator orientation="vertical" className="h-3 bg-muted-foreground mx-1" />
                                                                ) }
                                                                <span>Speak</span>
                                                            </>
                            ...
                            <CardContent className="space-y-3 sm:space-y-4">
                                { user.certificates?.length > 0 ? (
                                    user.certificates.map( ( certificate ) => (
                                        <div key={ certificate.id } className="font-medium text-foreground text-sm sm:text-base">
...
const ContactItem = ( { icon, label, value }: { icon: React.ReactNode, label: string, value: string } ) => (
    <div className="flex items-start gap-3">
        <div className="bg-muted p-1.5 sm:p-2 rounded-lg flex-shrink-0">
            { icon }
        </div>
