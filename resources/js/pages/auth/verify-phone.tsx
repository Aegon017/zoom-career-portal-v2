...
export default function VerifyPhone({ user }: Props) {
    ...
    const [step, setStep] = useState<Step>(props.step ?? (user.phone_verified_at ? 'verified' : 'request'));
                ...
                <Card className="w-full max-w-md shadow-lg">
                    <CardHeader className="text-center">
                        <CardTitle className="text-2xl">
                            {step === 'request' ? 'Enter Phone Number' : step === 'verify' ? 'Verify OTP' : 'Phone Verified'}
                        </CardTitle>
                    ...
                    <CardContent className="space-y-6">
                        {step === 'request' && (
                            <>
                                ...
                                <Button onClick={sendOtp} disabled={processing || !data.phone} className="w-full">
                                    Send OTP
                                </Button>
                            </>
                        )}
                            ...
                            <>
                                <div className="space-y-2 text-center">
                                    ...
                                    <div className="flex justify-center pt-4">
                                        <InputOTP maxLength={6} value={data.code} onChange={(val) => setData('code', val)}>
                                            <InputOTPGroup>
                                ...
                                <div className="text-muted-foreground flex items-center justify-between pt-2 text-sm">
                                    {timer > 0 ? (
                                        ...
                                        <Button variant="link" className="h-auto p-0 text-blue-500" onClick={sendOtp}>
                                            Resend OTP
                                        </Button>
                                    )}
                                    <Button onClick={verifyOtp} disabled={processing || data.code.length !== 6}>
                                        Verify OTP
                                    </Button>
                                </div>
                            </>
                        ...
                        {step === 'verified' && <p className="text-center text-green-600">Your phone number is verified!</p>}
                    </CardContent>
                </Card>
            </div>
        </>
