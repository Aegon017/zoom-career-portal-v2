		...
		if (last_page <= 5) {
			...
			if (current_page > 1)
				items.push(
					<PaginationItem key={current_page - 1}>
						<PaginationLink
							href={
								links.find((l) => l.label === String(current_page - 1))?.url ||
								"#"
							}
						>
							{current_page - 1}
			...
			items.push(
				<PaginationItem key={current_page}>
					<PaginationLink
						href={
							links.find((l) => l.label === String(current_page))?.url || "#"
						}
						isActive
					>
						{current_page}
						...
						<CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
							...
							<div className="space-y-2">
								...
								<Select
									...
									onValueChange={(value) =>
										handleFilterChange("employer", value)
									}
								>
							...
							([key, value]) =>
								value && (
									<Badge key={key} variant="secondary" className="gap-1 py-1">
										{key === "student" &&
											`Student: ${students.find((s) => s.id.toString() === value)?.name || value}`}
										{key === "employer" &&
											`Employer: ${employers.find((e) => e.id.toString() === value)?.name || value}`}
				...
				{chats.data.length === 0 ? (
					...
					<>
						{chats.data.map((chat) => {
							...
							const lastMessage =
								messageCount > 0 ? chat.messages[messageCount - 1] : null;
										...
										<div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-2 text-sm text-muted-foreground">
											...
											{lastMessage && (
												<div className="flex items-center">
													...
													<span>
														Last activity: {formatDate(lastMessage.created_at)}
													</span>
												</div>
											)}
										</div>
