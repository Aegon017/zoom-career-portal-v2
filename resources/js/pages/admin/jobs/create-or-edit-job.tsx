...
const CreateOrEditJob = ( {
	...
	const form = useForm( {
		defaultValues: {
			title: job?.title || "",
			skills:
				job?.skills.map( ( s ) => ( { value: String( s.id ), label: s.name } ) ) || [],
			...
			domain_id: job?.domain
				? [ { value: String( job.domain.id ), label: job.domain.name } ]
				: [],
			company_id: job?.company
				? [ { value: String( job.company.id ), label: job.company.name } ]
				: [],
			recruiter_id: job?.recruiter
				? [ { value: String( job.recruiter.id ), label: job.recruiter.name } ]
				: [],
		},
	} );
	...
	const onSubmit = ( data: any ) => {
		...
		const formData = {
			...
			domain_id: data.domain_id[ 0 ]?.value,
			company_id: data.company_id[ 0 ]?.value,
			recruiter_id: data.recruiter_id[ 0 ]?.value,
		};

		if ( operation === "Edit" && job?.id ) {
			router.put( `/admin/jobs/${ job.id }`, formData, {
	...
	const searchIndustries = async ( searchTerm: string ) => {
		const response = await axios.get( "/industries/search", {
			params: { search: searchTerm },
		} );
		return response.data;
	};
	...
	const searchDomains = async ( searchTerm: string ) => {
		try {
			const response = await axios.get( "/admin/domains/search", {
				params: { search: searchTerm },
			} );
	...
	const searchRecruiters = async ( searchTerm: string ) => {
		try {
			const response = await axios.get( "/recruiters/search", {
				params: { search: searchTerm },
			} );
	...
	useEffect( () => {
		if ( selectedDomain?.length > 0 ) {
			...
			axios
				.get( `/admin/domains/${ domainId }/skills` )
				.then( ( { data } ) => {
					...
					setValue( "skills", ( prev: Option[] ) => {
						...
						const newSkills = domainSkills.filter(
							( s: Option ) => !existingSkills.has( s.value )
						);
