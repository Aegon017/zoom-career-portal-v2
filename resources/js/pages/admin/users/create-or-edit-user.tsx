...
import { type BreadcrumbItem, type Option, type Role, type User } from '@/types';
...
export default function CreateOrEditUser( { user, roleOptions, operation }: Props ) {
    ...
    const form = useForm<FormData>( {
        defaultValues: {
            ...
            email: user?.email || '',
            password: '',
            roles: user?.roles?.map( role => ( { value: String( role.id ), label: role.name } ) ) || [],
        }
    } );
    ...
    const onSubmit = useCallback( ( formData: FormData ) => {
        const payload = {
            ...
            ...( operation.value === 'Edit' && !formData.password && { password: undefined } ),
        ...
        if ( operation.value === 'Create' ) {
            ...
            router.put( `/admin/users/${ user?.id }`, payload, { onError: handleErrors } );
        }
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <FormField
                                    ...
                                    render={ ( { field } ) => (
                                        <FormItem>
                                            <FormLabel>
                                                Password{ operation.value === 'Edit' && ' (Leave blank to keep current)' }
                                            </FormLabel>
                                            <FormControl>
                                                <Input
                                                    type="password"
                                                    placeholder={ operation.value === 'Edit' ? '••••••••' : '' }
                                                    { ...field }
                                                    value={ field.value || '' }
                                                />
                                            </FormControl>
