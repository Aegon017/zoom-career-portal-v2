$(document).ready(function () {
	...
	$(document).on(
		"click",
		".zc-dropdown-field > input.dropdown-toggle",
		function () {
		...
		if (file) {
			...
			$.ajax({
						...
						xhr.upload.addEventListener(
							...
							function (e) {
								if (e.lengthComputable) {
									...
									$(
										".zc-resume-upload-wrapper #progess-container #progress-bar",
									).css("width", percent + "%");
								}
							},
							false,
				...
				success: function (response) {
					$(".zc-resume-upload-wrapper #progess-container #progress-bar").css(
						"width",
						"100%",
					);
					setTimeout(function () {
	...
	$(
		".zc-lightbox-wrapper .zc-btn-pclose, .zc-lightbox-wrapper .lightbox-footer .btn-cancel",
	).on("click", function (e) {
		e.preventDefault();
	...
	$("#key-skill-search").on("input", function () {
		...
		const matches = skills_data.filter(
	...
	$(document).on(
		"click",
		".filter-dropdown-wrapper ul.search-list li",
		function () {
			...
			if ($selecteditems.find('.tag[data-value="' + value + '"]').length > 0)
				return;

			$selecteditems.append(
				'<div class="tag" data-value="' +
					value +
					'">' +
					label +
	...
	$(".zc-chat-content .zc-chat-common-header .left .back-btn").on(
		"click",
		function () {
