...
export function DataTable<TData, TValue>( {
	...
	const isColumnSortable = useCallback( ( columnId: string | undefined ) => {
		if ( !columnId ) return false;
		return columns.some( col => ( col as any ).accessorKey === columnId && col.enableSorting !== false );
	}, [ columns ] );
	...
	useEffect( () => {
		...
		const validSorting = sorting && isColumnSortable( sorting.column );
		...
		router.get(
			routeName,
			{
				...
				sortColumn: validSorting ? sortColumn : undefined,
				sortDirection: validSorting ? sortDirection : undefined,
			},
			{
				preserveState: true,
				replace: true,
	...
	}, [ debouncedSearch, pagination.per_page, sorting, routeName, isColumnSortable ] );
	...
	}, [ globalFilter, pagination.per_page, routeName, sorting, isColumnSortable ] );
	...
	const handleExportFieldToggle = useCallback( ( key: string ) => {
	...
	const selectAllExportFields = useCallback( ( select: boolean ) => {
		setSelectedExportFields( prev =>
	...
	}, [ selectedExportFields, exportUrl ] );
	...
	const allFieldsSelected = selectedExportFields.every( field => field.selected );
						...
						{ table.getHeaderGroups().map( ( headerGroup ) => (
							<TableRow key={ headerGroup.id }>
								{ headerGroup.headers.map( ( header ) => {
									...
									const isSorted = sorting?.column === header.column.id;
										...
										<TableHead key={ header.id }>
											{ isSortable ? (
												<Button
													...
													onClick={ () => {
														setSorting( ( prev ) => {
															if ( prev?.column === header.column.id ) {
																const newDirection = prev.direction === "asc" ? "desc" : "asc";
																return { column: header.column.id, direction: newDirection };
															}
															return { column: header.column.id, direction: "asc" };
														} );
													} }
												>
