...
export default function EmploymentModal({
	...
	useEffect(() => {
		if (isActive) {
			const isManual =
				!defaultValues?.company_id && !!defaultValues?.company_name;
			setManualEntry(isManual);

			if (defaultValues?.company_id && defaultValues?.company_name) {
				setSelectedCompany({
					label: defaultValues.company_name,
					value: defaultValues.company_id,
				});
			} else {
	...
	const onCompanyChange = (option: CompanyOption | null) => {
		...
		setValue("company_id", option ? option.value : null, {
			shouldValidate: true,
		});
		...
		>
			...
			>
				<div className="mb-3">
					{!manualEntry ? (
						<>
							...
							/>
							{errors.company_id && (
								<p className="text-danger mt-1">Company is required</p>
							)}
						</>
					) : (
						<>
				...
				<div className="mb-3">
					...
					/>
					{errors.title && (
						<p className="text-danger mt-1">Job title is required</p>
					)}
				</div>
