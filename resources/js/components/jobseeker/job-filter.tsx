...
const JobFilters = ({
	...
	const [isEmploymentDropdownOpen, setIsEmploymentDropdownOpen] =
		useState(false);
	...
	return (
		<div className="zc-jobs-filter-wrapper zc-card">
			...
			<div className="zc-job-filter-widget d-block position-relative w-100">
				...
				<div
					ref={companyRef}
					className="zc-field-wrap zc-dropdown-field position-relative mb-2"
				>
					...
					<div
						...
						style={{ display: isCompanyDropdownOpen ? "block" : "none" }}
					>
						<input
							type="text"
						...
						<div className="dropdown-options">
							{filteredCompanies.length === 0 ? (
								<div className="option py-2 text-center">
									No companies found
								</div>
							) : (
								filteredCompanies.map((company) => (
									<label className="option" key={company.id}>
										<input
											type="checkbox"
											checked={data.selected_companies.includes(
												company.id.toString(),
											)}
											onChange={() =>
												toggleFilter(
				...
				>
					...
					<input
						...
						readOnly
						value={
							data.industries.length ? `${data.industries.length} selected` : ""
						}
						onClick={() => {
								...
								filteredIndustries.map((industry) => (
									<label className="option" key={industry.id}>
										<input
											...
											onChange={() =>
												toggleFilter("industries", industry.id.toString())
											}
										/>
										{industry.name}
									</label>
				...
				>
					...
					<input
						...
						readOnly
						value={
							data.locations.length ? `${data.locations.length} selected` : ""
						}
						onClick={() => {
					...
					<div
						className="zc-dropdown"
						style={{ display: isLocationDropdownOpen ? "block" : "none" }}
					>
						<input
							type="text"
							className="zc-dropdown-search"
								...
								filteredLocations.map((location) => (
									<label className="option" key={location.id}>
										<input
											...
											onChange={() =>
												toggleFilter("locations", location.id.toString())
											}
										/>
										{location.full_name}
									</label>
