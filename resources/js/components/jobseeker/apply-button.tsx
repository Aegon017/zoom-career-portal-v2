...
export default function ApplyButton({ jobId, hasApplied, status }: ApplyButtonProps) {
    ...
    return (
        <>
            {hasApplied ? (
                <button onClick={handleWithdraw} disabled={isProcessing} className="zc-btn zc-btn-primary">
                    {isProcessing ? 'Processing...' : `Withdraw${status ? ` (${status})` : ''}`}
                </button>
            ) : (
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    <DialogTrigger asChild>
                        <button disabled={isProcessing} className="zc-btn zc-btn-primary" type="button">
                    ...
                    <DialogContent>
                        ...
                        <Form {...form}>
                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                                ...
                                <DialogFooter>
                                    <button type="submit" className="zc-btn zc-btn-primary" disabled={isProcessing || (!resume && !selectedResumeId)}>
