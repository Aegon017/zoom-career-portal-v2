...
const SummaryModal: React.FC<SummaryModalProps> = ({ isActive, handleClose, defaultSummary }) => {
    ...
    return (
        <PopupModal isActive={isActive} title="Profile Summary" onClose={handleClose} onSave={handleSubmit(onSubmit)}>
            <p className="mb-0">
                Give recruiters a brief overview of the highlights of your career, key achievements, and career goals to help recruiters know your
                profile better.
            </p>
            <div className="modal-body overflow-auto px-0" style={{ maxHeight: '400px' }}>
                <div className="mb-3">
                    {/* <label htmlFor="summary" className="form-label"></label> */}
                    <textarea className="form-control" {...register('summary', { required: true })} rows={6} />
                </div>
            </div>
        </PopupModal>
    );
