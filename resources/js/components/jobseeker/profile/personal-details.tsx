...
export default function PersonalDetails({ isUpdatable, user }: Props) {
    ...
    const {
        ...
        defaultValues: {
            ...
            date_of_birth: user.personal_detail?.date_of_birth ? format(new Date(user.personal_detail.date_of_birth), 'yyyy-MM-dd') : '',
    ...
    useEffect(() => {
        ...
        axios.get('/locations/search', { params: { search: locationSearch } }).then((res) => {
            const results = res.data.map((loc: any) => ({
                label: loc.label,
                value: loc.value.toString(),
            }));

            if (defaultLocation && !results.find((opt: any) => opt.value === defaultLocation.value)) {
    ...
    const onSubmit = (data: FormData) =>
        router.post(
            '/jobseeker/profile/personal-details',
            { ...data },
            {
        ...
        <>
            <div className="zc-profile-personal-details zc-card-style-2 mb-3">
                <div className="zc-card-header d-flex justify-content-between align-items-center">
                    ...
                    {isUpdatable && <EditButton onClick={openModal} id="edit-personal" ariaLabel="Edit personal details" />}
                </div>
                <div className="zc-card-content">
                    <div className="row candidate-personal-details">
                        {[
                            ...
                            ['Work Permit', user.work_permits?.length ? user.work_permits.map((wp) => wp.country).join(', ') : null],
                            ['Differently Abled', user.personal_detail?.differently_abled ? 'Yes' : 'No'],
                        ].map(([title, value]) => (
            ...
            <PopupModal title="Personal Details" isActive={activeModal === 'personalDetails'} onClose={closeModal} onSave={handleSubmit(onSubmit)}>
                <div className="mb-3">
                    <label>Gender</label>
                    <Controller
                        name="gender"
                ...
                <div className="mb-3">
                    ...
                    />
                    {errors.location_id && <p className="text-danger">{errors.location_id.message}</p>}
                </div>
                ...
                <div className="mb-3">
                    ...
                    />
                    {errors.marital_status && <p className="text-danger">{errors.marital_status.message}</p>}
                </div>
                ...
                <div className="mb-3">
                    ...
                    {errors.work_permit && <p className="text-danger">{errors.work_permit.message}</p>}
                </div>
