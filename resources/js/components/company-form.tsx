...
const CompanyForm = ({
	...
	const handleFileUpload = useCallback(
		(field: keyof FormValues) => (tempPath: string) => {
	...
	const renderRadioOptions = useCallback(
		(options: Option[], field: any, name: string) => (
			<RadioGroup
				...
				className="mt-2 flex flex-wrap gap-x-1 gap-y-4"
			>
				{options.map((option, index) => {
					...
					return (
						<div key={id}>
							...
							<Label
								htmlFor={id}
								className={`cursor-pointer rounded-sm border px-4 py-2 text-center text-sm font-medium transition-colors ${
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormDescription>
								This email is <strong>mandatory</strong> for company
								verification.
							</FormDescription>
							<FormMessage />
						</FormItem>
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormDescription>
								This phone number is <strong>mandatory</strong> for company
								verification.
							</FormDescription>
							<FormMessage />
						</FormItem>
